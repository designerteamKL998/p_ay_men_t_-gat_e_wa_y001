...
const bankTransferOptions = [
    "A zt best 8950 - A zt best 8950",
    "C zt best 8950 - C zt best 8950",
    "Affin Bank - YUME SNACK ENTERPRISE",
    "All Currency Bank - testing",
    "BITBOOK ONLINE MARKETING - BITBOOK ONLINE MARKETING",
    "CREDIT RHB BANK SDN BHD - CREDIT RHB BANK SDN BHD"
];
...
function renderTabs() {
    const methodIcons = {
        online: "assets/onlinetransfer.svg",
        qr: "assets/duitNow.svg",
        bank: "assets/bankIn.svg",
        wallet: "assets/touchNgo.svg",
        crypto: "assets/tether.svg",
    };
    $("#categoryTabs").innerHTML = Object.entries(methods)
        .map(
            ([id, m]) =>
                `<button class="category-tab ${state.method === id ? "active" : ""}" onclick="setMethod('${id}')"><img src="${methodIcons[id]}" class="category-tab-icon" alt=""><span>${m.label}</span></button>`
        )
        .join("");
}
function renderChannels() {
    ...
    const packageSection = `<div id="depositPackageSelection" class="deposit-package-selection">
<label class="form-label">Package <span class="required">*</span></label>
<div class="deposit-package-wrap">
<button type="button" class="deposit-package-select" onclick="toggleDepositPackageDropdown()">
<span id="depositPackageText" class="${state.package ? "" : "package-placeholder"}">${state.package || "Please Select"}</span>
<span class="package-arrow">▼</span>
</button>
<div id="depositPackageDropdown" class="deposit-package-dropdown">
${depositPackages
    .filter((p) => p !== "Please Select")
    .map((p) => `<button type="button"class="deposit-package-item"onclick="chooseDepositPackage('${p}')">${p}</button>`)
    .join("")}
</div>
</div>
</div>`;
    const channelTitle = $("#channelTitle");
    if (channelTitle) channelTitle.insertAdjacentHTML("beforebegin", packageSection);
    if(state.method==="bank"){
        ...
        <button type="button" class="bank-select" onclick="toggleReceivingBankDropdown(event)">
        <span>${state.bank||"Please Select"}</span>
        <span>▼</span>
        </button>

        <div id="receivingBankDropdown" class="bank-dropdown">
        <div id="receivingBankOptions">
        ${bankTransferOptions.map(bank=>`
        <button
        type="button"
        class="bank-option ${state.bank===bank?"selected":""}"
        onclick='chooseBank(${JSON.stringify(bank)});return false;'
        >
        ${bank}
            </button>
        `).join("")}
    ...
    $("#channelTitle").textContent = "Payment channel";
    $("#filterBtn").classList.add("hidden");
    $("#filterRow").classList.add("hidden");
    ...
    <div id="paymentChannelSelection" class="payment-channel-selection">
        ...
        <div class="payment-channel-wrap">
            ...
            <button
                ...
                onclick="togglePaymentChannelDropdown()"
            >
                <span id="paymentChannelText">
                    ${state.channelSelected?channels[state.channel]?.name:"Please Select"}
                </span>

                <span class="payment-channel-arrow">▼</span>
            </button>

            <div
                id="paymentChannelDropdown"
                class="payment-channel-dropdown"
            >
                ...
                ${list.map(id=>{
                    ...
                    >
                        <span class="payment-channel-item-logo">
                            ${logo}
                        </span>
    ...
    `;
        ...
        $("#channelGrid").innerHTML=paymentChannelSection;
    }
    else {
        $("#channelGrid").classList.remove("ewallet-paged");
        const paymentChannelSection = `
...
${list
    .map((id) => {
        ...
        return `<button type="button"class="payment-channel-item"onclick="choosePaymentChannel('${id}')"><span class="payment-channel-item-logo">${logo}</span><span>${c.name}</span></button>`;
    })
    .join("")}
...
`;
        ...
        if(state.method==="crypto"){
            const networkSection=`<div id="cryptoNetworkSelection" style="grid-column:1/-1;width:100%;box-sizing:border-box"><div style="font-weight:600;color:#10243f;margin-bottom:15px">Choose a crypto network</div><div class="crypto-network-wrap" style="position:relative;width:100%"><button type="button" class="crypto-network-select" onclick="toggleCryptoNetworkDropdown()" style="width:100%;height:52px;padding:0 18px;border:1px solid #b9cce3;border-radius:8px;background:#fff;color:#10243f;font-size:16px;text-align:left;display:flex;align-items:center;justify-content:space-between;cursor:pointer"><span id="cryptoNetworkText">${state.cryptoNetwork||"Please Select"}</span><span>▼</span></button><div id="cryptoNetworkDropdown" class="crypto-network-dropdown" style="display:none;position:absolute;top:calc(100% + 4px);left:0;width:100%;background:#fff;border:1px solid #d9e2ee;border-radius:8px;z-index:100;overflow:hidden"><button type="button" onclick="setCryptoNetwork('TRC20-USDT');closeCryptoNetworkDropdown()" style="width:100%;padding:14px 18px;border:0;background:#fff;text-align:left;cursor:pointer;display:flex;align-items:center;gap:12px"><img src="assets/tether.svg" style="width:30px;height:30px;object-fit:contain"><span>TRC20-USDT</span></button><button type="button" onclick="setCryptoNetwork('ERC20-USDT');closeCryptoNetworkDropdown()" style="width:100%;padding:14px 18px;border:0;background:#fff;text-align:left;cursor:pointer;display:flex;align-items:center;gap:12px"><img src="assets/ethereum.svg" style="width:30px;height:30px;object-fit:contain"><span>ERC20-USDT</span></button></div></div></div>`;
...
function chooseDepositPackage(packageName) {
    ...
    const text = $("#depositPackageText");
    if (text) {
        text.textContent = packageName;
        text.classList.remove("package-placeholder");
    }
    const dropdown = $("#depositPackageDropdown");
    if (dropdown) dropdown.classList.remove("show");
    validate();
...
depositPackageStyle.textContent = `.deposit-package-selection{margin-bottom:28px}.deposit-package-wrap{position:relative;width:100%}.deposit-package-select{width:100%;height:44px;padding:0 14px;border:1px solid #b9d0ff;border-radius:6px;background:#fff;color:#10243f;display:flex;align-items:center;justify-content:space-between;font-size:16px;cursor:pointer;text-align:left;box-sizing:border-box}.package-placeholder{color:#777}.package-arrow{color:#627795;font-size:14px}.deposit-package-dropdown{position:absolute;top:calc(100% + 4px);bottom:auto;left:0;right:0;background:#fff;border:1px solid #d9e2ee;border-radius:6px;box-shadow:0 8px 24px rgba(20,50,90,.15);max-height:240px;overflow-y:auto;z-index:9999;display:none}.deposit-package-dropdown.show{display:block}.deposit-package-item{width:100%;min-height:44px;padding:10px 14px;border:0;background:#fff;color:#10243f;text-align:left;font-size:15px;cursor:pointer}.deposit-package-item:hover{background:#f1f6ff}`;
...
depositHelpStyle.textContent = `.deposit-title-with-help{display:inline-flex;align-items:center;gap:9px}.deposit-title-with-help h1,.deposit-title-with-help h2,.deposit-title-with-help h3{margin-right:0}.deposit-help{position:relative;display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px;cursor:help;vertical-align:middle}.deposit-help img{width:22px;height:22px;display:block;cursor: pointer}.deposit-help>span{position:absolute;left:32px;top:50%;transform:translateY(-50%);background:#10243f;color:#fff;padding:7px 11px;border-radius:6px;font-size:13px;font-weight:500;line-height:1.2;white-space:nowrap;opacity:0;visibility:hidden;pointer-events:none;transition:opacity .18s ease;z-index:1000}.deposit-help:hover>span{opacity:1;visibility:visible}`;
